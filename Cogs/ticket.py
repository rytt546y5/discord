...
class TicketActionView(discord.ui.View):
    ...
    async def close_confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def confirm_callback(it: discord.Interaction):
            if not can_manage_ticket(it, self.staff_role_id): return
    ...
    async def execute_close(self, interaction: discord.Interaction, reason="特になし"):
        await interaction.response.send_message("⌛ ログを生成してDMへ送信中...", ephemeral=True)
        ...
        html_str = generate_html_transcript(interaction.channel.name, str(owner) if owner else "不明", str(self.claimed_by) if self.claimed_by else "未担当", str(interaction.user), messages, reason)
        ...
        # --- 送信先の整理（重複防止） ---
        targets = [interaction.user] # まず実行者をリストに入れる
        if owner and owner.id != interaction.user.id:
            targets.append(owner) # 作成者が実行者と違う場合のみリストに追加
...
class TicketCog(commands.Cog):
    ...
    async def ticket_panel(self, interaction: discord.Interaction, channel: discord.TextChannel, staff_role: discord.Role, title: str = "🎫 サポートチケット", description: str = "下のボタンを押してチケットを作成してください。"):
        ...
        data[str(interaction.guild.id)] = {"staff_role_id": staff_role.id}
...
class TicketView(discord.ui.View):
    ...
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(title="🎟 お問い合わせ受領", description=f"{user.mention} 様、ご用件をお書きください。", color=discord.Color.green())
