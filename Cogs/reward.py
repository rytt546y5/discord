...
class Reward(commands.Cog):
    ...
    async def reward_stock(self, interaction: discord.Interaction, name: str, file: discord.Attachment):
        ...
        await interaction.response.send_message(f"✅ 「{name}」に在庫を追加しました（現在の在庫数: {len(items[gid][name]['stock'])}）", ephemeral=True)
    ...
    async def reward_list(self, interaction: discord.Interaction):
        ...
        for name, data in guild_items.items():
            if data["mode"] == "infinite":
                ...
                stock_text = f"在庫: {len(data['stock'])}件"
            
            embed.add_field(name=name, value=f"モード: {data['mode']}\n{stock_text}", inline=False)

        await interaction.response.send_message(embed=embed, ephemeral=True)

    @app_commands.command(name="reward_panel", description="受取パネルを送信します")
    ...
    @app_commands.command(name="reward_log_set", description="ログ送信先チャンネルを設定します")
